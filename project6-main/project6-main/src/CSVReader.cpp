...
std::vector<std::string> CSVReader::readRow(std::string &csv_line) {
    std::vector<std::string> row = {};

    std::stringstream stream(csv_line);
    std::string cell;
...
InterferenceGraph<Variable> CSVReader::load(const std::string &graph_path) {
    InterferenceGraph<Variable> igraph;
    std::string line;
    ...
    while (std::getline(file_stream, line)) {
        ...
        if (row.size() > 2) {
            throw std::runtime_error(
                "Graph contains row with more than two vertices: " +
                graph_path);
        }

        for (const auto &vertex : row) {
            igraph.addVertex(vertex);
        }

        if (row.size() == 2) {
            igraph.addEdge(row.at(0), row.at(1));
        }
    }
